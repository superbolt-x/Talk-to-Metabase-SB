...
| | `POST /api/card/` | `create_card` | ✅ Implemented (with visualization settings support) |
| | `PUT /api/card/{id}` | `update_card` | ✅ Implemented (with visualization settings support) |
...
| **Visualization Operations** | N/A (Documentation Tool) | `GET_VISUALIZATION_DOCUMENT` | ✅ Implemented |
...
## Visualization Settings Tools

Talk to Metabase provides comprehensive visualization settings tools that enable Claude to create and modify cards with custom chart configurations. The visualization system follows a documentation-first approach to ensure correct syntax and validation.

### Overview

The visualization tools address a key challenge: Claude doesn't have *a priori* knowledge of Metabase's complex visualization settings syntax. The solution is a documentation-first approach:

1. **GET_VISUALIZATION_DOCUMENT**: Provides comprehensive documentation for specific chart types
2. **Enhanced card tools**: `create_card` and `update_card` support visualization settings with validation
3. **Internal validation**: Ensures settings match chart type requirements

### Supported Chart Types

The visualization system supports four core chart types:

- **table**: Data tables with column formatting, sorting, and conditional formatting
...
### GET_VISUALIZATION_DOCUMENT Tool

This tool provides filtered documentation for specific chart types, ensuring Claude has the exact syntax needed for visualization settings.

#### Key Features

1. **Filtered Documentation**: Returns only documentation for requested chart types
2. **Comprehensive Coverage**: Includes common settings, chart-specific settings, and complete examples
3. **Validation Guidelines**: Provides specific validation rules and best practices
4. **Multiple Chart Support**: Can document multiple chart types in a single call
...
```json
{
  ...
  "validation_notes": [
    "Field references must use exact format: [\"ref\",[\"field\",123,null]]",
...
Both `create_card` and `update_card` tools have been enhanced to support visualization settings.

#### Create Card with Visualization Settings
...
The visualization system includes comprehensive validation to prevent errors:

#### Chart Type Validation

Both `create_card` and `update_card` validate that the `display` parameter is one of the supported chart types:
...
The `validate_visualization_settings` function performs comprehensive validation:

1. **Required Fields**: Ensures chart-specific required fields are present
2. **Valid Values**: Validates enum values for settings like axis scales
3. **Field References**: Checks proper formatting of field reference strings
4. **Color Values**: Validates hex color codes
5. **Boolean/Numeric Types**: Ensures proper data types

#### Validation Examples

**Valid Bar Chart Settings:**
```json
{
  "graph.dimensions": ["Category"],
  "graph.metrics": ["Revenue"],
...
{
  "graph.show_values": true
  // Missing required graph.dimensions and graph.metrics
}
```
...
```json
{
  ...
  "validation_errors": [
    "Bar charts require 'graph.dimensions' to be specified",
    "Bar charts require 'graph.metrics' to be specified"
  ],
  "suggestion": "Call GET_VISUALIZATION_DOCUMENT with chart_types=['bar'] to see correct syntax"
}
```
...
Table charts support column formatting and conditional formatting:

- **table.columns**: Configure which columns to display
- **column_settings**: Per-column formatting (currency, dates, colors)
- Field references use format: `["ref",["field",123,null]]` or `["name","Column Name"]`

#### Bar Charts

Bar charts support stacking, axis configuration, and series customization:

- **graph.dimensions**: X-axis categories
- **graph.metrics**: Y-axis values
- **stackable.stack_type**: null, "stacked", or "normalized"
- **series_settings**: Per-series colors and names

#### Line Charts

Line charts support interpolation, markers, and trend lines:

- **graph.x_axis.scale**: "linear", "log", "pow", or "timeseries"
- **series_settings**: Line interpolation and marker configuration
- **graph.show_trendline**: Boolean for trend line display

#### Combo Charts

Combo charts mix different chart types and require series_settings:

- **series_settings**: REQUIRED - each series must specify:
  - **display**: "line", "bar", or "area"
  - **axis**: "left" or "right"
- **graph.y_axis.auto_split**: Automatically create separate Y-axes

### Implementation Details

#### Tool Loading

Visualization tools are always loaded automatically:
...
The visualization system uses a layered error handling approach:

1. **Chart Type Validation**: Immediate rejection of invalid display types
2. **Settings Validation**: Comprehensive validation with detailed error messages
3. **Graceful Fallback**: If visualization tools are unavailable, log warning but continue
4. **Helpful Suggestions**: Error messages include suggestions to call documentation tool

#### Conditional Imports

To avoid circular imports, validation is imported conditionally:

```python
try:
    from .visualization import validate_visualization_settings
    is_valid, validation_errors = validate_visualization_settings(settings, display)
except ImportError:
    logger.warning("Visualization validation not available")
except Exception as e:
...
1. **Always validate chart types** before processing visualization settings
2. **Use conditional imports** to avoid circular dependencies
3. **Provide helpful error messages** that guide users to the documentation tool
4. **Log validation events** for debugging and monitoring

#### For Documentation

1. **Include complete examples** for each chart type
2. **Document required vs optional fields** clearly
3. **Provide validation notes** for complex field reference formats
4. **Show common patterns** like field references and series settings

#### For Testing

1. **Test both valid and invalid** chart types
2. **Verify validation error messages** are helpful
3. **Test edge cases** like missing required fields
4. **Ensure graceful fallback** when tools are unavailable

### Usage Workflow

The recommended workflow for using visualization settings:

1. **Determine chart type**: Choose from table, bar, line, or combo
2. **Get documentation**: Call `GET_VISUALIZATION_DOCUMENT` with the chart type
3. **Construct settings**: Build visualization_settings object using documentation
4. **Create/update card**: Use `create_card` or `update_card` with settings
5. **Handle validation errors**: If validation fails, refer back to documentation

### Example: Complete Bar Chart Creation

```python
# Step 1: Get documentation
response = await get_visualization_document(["bar"], ctx)
docs = json.loads(response)

# Step 2: Construct settings based on documentation
visualization_settings = {
...
# Step 3: Create card with visualization settings
result = await create_card(
    database_id=195,
    query="SELECT product_category, SUM(sales) as total_sales FROM orders GROUP BY product_category",
...
This comprehensive visualization system provides Claude with the tools needed to create sophisticated Metabase visualizations while ensuring correct syntax through documentation and robust validation.
```
...
9. **Visualization System Enhancements**:
   - Adding support for additional chart types (pie, scatter, gauge, etc.)
   - Implementing advanced visualization features (annotations, custom colors palettes)
   - Adding field reference validation against actual card data
...
The comprehensive visualization settings system enables Claude to create sophisticated Metabase charts with proper validation and documentation guidance. The documentation-first approach ensures correct syntax while the validation system prevents common errors.

Remember to maintain consistency with the existing code patterns and to thoroughly test your implementations, especially for visualization settings which require careful validation of complex nested objects and field references.
